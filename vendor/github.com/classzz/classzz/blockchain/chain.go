...
// exceeded.
func (b *BlockChain) addOrphanBlock(block *czzutil.Block) {
...
func (b *BlockChain) calcSequenceLock(node *blockNode, tx *czzutil.Tx, utxoView *UtxoViewpoint, mempool bool) (*SequenceLock, error) {
...
//
// This function MUST be called with the chain state lock held (for writes).
func (b *BlockChain) connectBlock(node *blockNode, block *czzutil.Block,
...
//
// This function MUST be called with the chain state lock held (for writes).
func (b *BlockChain) disconnectBlock(node *blockNode, block *czzutil.Block, view *UtxoViewpoint) error {
...
func countSpentOutputs(block *czzutil.Block) int {
...
func (b *BlockChain) reorganizeChain(detachNodes, attachNodes *list.List) error {
	...
	detachBlocks := make([]*czzutil.Block, 0, detachNodes.Len())
	detachSpentTxOuts := make([][]SpentTxOut, 0, detachNodes.Len())
	attachBlocks := make([]*czzutil.Block, 0, attachNodes.Len())
...
//
// This function MUST be called with the chain state lock held (for writes).
func (b *BlockChain) connectBestChain(node *blockNode, block *czzutil.Block, flags BehaviorFlags) (bool, error) {
...
func (b *BlockChain) isCurrent() bool {
	...
	//fmt.Println(b.bestChain.Tip().height,b.bestChain.Tip().timestamp,minus24Hours)
...
type IndexManager interface {
	...
	// required.
	ConnectBlock(database.Tx, *czzutil.Block, []SpentTxOut) error

	// DisconnectBlock is invoked when a block has been disconnected from
	// the main chain. The set of outputs scripts that were spent within
	// this block is also returned so indexers can clean up the prior index
	// state for this block.
	DisconnectBlock(database.Tx, *czzutil.Block, []SpentTxOut) error
}
...
func New(config *Config) (*BlockChain, error) {
	...
	var hecoclients []*rpc.Client
	for _, hecorpc := range config.HecoRPC {
		// Connect to local bitcoin core RPC server using HTTP POST mode.

		if hecorpc[:4] != "http" {
			hecorpc = "http://" + hecorpc
		}
		client, err := rpc.Dial(hecorpc)
		if err != nil {
			log.Warn(fmt.Printf("rpc:[failed][url:%s][err:%v]", hecorpc, err))
		}
		...
		log.Warnf("rpc test:[successed][url:%s][block:%d]", hecorpc, number)
